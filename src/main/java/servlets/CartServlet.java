...
public class CartServlet extends HttpServlet {
    ...
    if (!StoreUtil.isLoggedIn(UserRole.CUSTOMER, req.getSession())) {
        handleCustomerNotLoggedIn(req, res, pw);
        return;
    }
    try {
        ...
        displayCartHeader(pw);
        List<Book> books = bookService.getBooksByCommaSeparatedBookIds(bookIds);
        double amountToPay = displayCartItems(session, pw, books);

        displayCartFooter(pw, amountToPay);
...
private void handleCustomerNotLoggedIn(HttpServletRequest req, HttpServletResponse res, PrintWriter pw) throws ServletException, IOException {
    RequestDispatcher rd = req.getRequestDispatcher("CustomerLogin.html");
    rd.include(req, res);
    pw.println("<table class=\"tab\"><tr><td>Please Login First to Continue!!</td></tr></table>");
}

private void displayCartHeader(PrintWriter pw) {
...
private double displayCartItems(HttpSession session, PrintWriter pw, List<Book> books) {
...
private void displayCartFooter(PrintWriter pw, double amountToPay) {
    if (amountToPay > 0) {
        pw.println("    <tr style='background-color:green'>\r\n"
